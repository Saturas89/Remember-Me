...
describe('useReminder - Enhanced REQ-016 Features', () => {
  ...
  describe('backoff stage progression', () => {
    it('progresses backoffStage 0 → 1 after first trigger', async () => {
      const initialState = JSON.stringify({
        permission: 'enabled',
        backoffStage: 0
      })
      mockLocalStorage.getItem.mockReturnValue(initialState)
    ...
    it('progresses backoffStage 1 → 2 after second trigger', async () => {
      const stage1State = JSON.stringify({
        permission: 'enabled',
        backoffStage: 1,
        lastShownAt: Date.now() - (4 * 24 * 60 * 60 * 1000) // 4 days ago
      })
      mockLocalStorage.getItem.mockReturnValue(stage1State)
    ...
    it('progresses backoffStage 2 → 3 after third trigger', async () => {
      const stage2State = JSON.stringify({
        permission: 'enabled',
        backoffStage: 2,
    ...
    it('stops scheduling after backoffStage 3 (24+ days)', async () => {
      ...
      // Should not schedule new notification after stage 3
  ...
  describe('badge API integration', () => {
    it('calls setAppBadge when enabled and open questions exist', async () => {
      const enabledState = JSON.stringify({ permission: 'enabled' })
      mockLocalStorage.getItem.mockReturnValue(enabledState)
      ...
      // Mock that there are 3 open questions
      await act(async () => {
        await (window.navigator as { setAppBadge: (count: number) => Promise<void> }).setAppBadge(3)
      })

      expect(window.navigator.setAppBadge).toHaveBeenCalledWith(3)
    })

    it('calls clearAppBadge when app is opened', async () => {
      installEnhancedNotification('granted')
      const useReminder = await loadEnhancedHook()
      renderHook(() => useReminder())

      await act(async () => {
        await (window.navigator as { clearAppBadge: () => Promise<void> }).clearAppBadge()
      })

      expect(window.navigator.clearAppBadge).toHaveBeenCalled()
    })

    it('gracefully handles missing badge API', async () => {
      // Remove badge API
  ...
  describe('iOS fallback behavior', () => {
    it('does not schedule OS notifications when showTrigger unavailable', async () => {
      // iOS-like environment without showTrigger
      const requestPermission = vi.fn().mockResolvedValue('granted')
      const proto = {} // No showTrigger property
      const Notif = Object.assign(
        function Notification() { /* no-op */ },
        { permission: 'granted' as NotificationPermission, requestPermission, prototype: proto }
      )
      ;(globalThis as unknown as { Notification: unknown }).Notification = Notif
      ;(window as unknown as { Notification: unknown }).Notification = Notif
      ...
      // Should not attempt to schedule notification without showTrigger
      expect(mockServiceWorkerRegistration.showNotification).not.toHaveBeenCalled()
    })
  })
})